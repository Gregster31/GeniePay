    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl bg-[#15141a] border border-[#2e2d38]">
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-[#1a1821] border-b border-[#2e2d38]">
          <div>
            <h2 className="text-sm font-semibold text-white">Import Employees</h2>
            <p className="text-xs text-[#6f6b77] mt-0.5">{stepLabel[step]}</p>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          {step === ImportStep.Upload && (
            <>
              ...
              <button
                ...
                className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-[#1a1821] border border-[#2e2d38] hover:border-purple/30 transition-all group"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-xs text-[#6f6b77] mt-0.5 font-mono">name, email, walletAddress, role, department, payUsd</p>
                  </div>
                </div>
                <Download className="w-4 h-4 text-[#6f6b77] group-hover:text-purple-400 group-hover:translate-y-0.5 transition-all" />
              </button>
              ...
              <div
                ...
                className={[
                  ...
                  isDragging
                    ...
                    : 'bg-[#1a1821] border-2 border-dashed border-[#2e2d38] hover:border-purple/30 hover:bg-purple/[0.03]',
                ].join(' ')}
              >
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${isDragging ? 'bg-purple/20 border border-purple/30' : 'bg-white/[0.04] border border-[#2e2d38]'}`}>
                  <Upload className={`w-5 h-5 transition-colors ${isDragging ? 'text-purple-400' : 'text-[#6f6b77]'}`} />
                </div>
                <div className="text-center">
                  <p className="text-sm font-medium text-white">Drop your file here</p>
                  <p className="text-xs text-[#6f6b77] mt-1">or <span className="text-purple-400">click to browse</span></p>
                </div>
                <input
                  ref={fileInputRef}
                  type="file"
              ...
              <p className="text-xs text-[#44414c] text-center">
                Supports <span className="text-[#6f6b77] font-mono">.csv, .xlsx, .xls</span>
                {' · '}Required: <span className="text-[#6f6b77] font-mono">name, walletAddress, role, payUsd</span>
              </p>
            </>
          )}
          ...
          {step === ImportStep.Preview && (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 rounded-xl bg-[#1a1821] border border-[#2e2d38]">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[11px] font-semibold uppercase tracking-widest text-[#6f6b77]">Valid</span>
                  </div>
                  <p className="text-2xl font-bold text-white">{valid.length}</p>
                  <p className="text-xs text-[#6f6b77] mt-1">Ready to import</p>
                </div>
                <div className="p-4 rounded-xl bg-[#1a1821] border border-[#2e2d38]">
                  <div className="flex items-center gap-2 mb-2">
                    <AlertCircle className={`w-4 h-4 ${errors.length ? 'text-red-400' : 'text-[#44414c]'}`} />
                    <span className="text-[11px] font-semibold uppercase tracking-widest text-[#6f6b77]">Skipped</span>
                  </div>
                  <p className={`text-2xl font-bold ${errors.length ? 'text-red-400' : 'text-white'}`}>{errors.length}</p>
                  <p className="text-xs text-[#6f6b77] mt-1">Will be skipped</p>
                </div>
              </div>

              {/* File name */}
              {fileName && (
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-[#1c1b22] border border-[#2e2d38]">
                  <FileText className="w-3.5 h-3.5 text-[#6f6b77] shrink-0" />
                  <span className="text-xs text-[#c4bfce] font-mono truncate">{fileName}</span>
                </div>
              )}
              ...
              {valid.length > 0 && (
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-widest text-[#6f6b77] mb-2">Employees to import</p>
                  <div className="rounded-xl border border-[#2e2d38] overflow-hidden divide-y divide-[#2e2d38]/60">
                    {valid.slice(0, 5).map((emp, i) => (
                      <div key={i} className="flex items-center justify-between px-4 py-3 bg-[#1a1821]">
                        <div>
                          <p className="text-sm font-medium text-white">{emp.name}</p>
                          <p className="text-xs text-[#6f6b77]">{emp.role}{emp.department ? ` · ${emp.department}` : ''}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs font-mono text-[#8b6cf7]">{emp.walletAddress.slice(0, 6)}…{emp.walletAddress.slice(-4)}</p>
                          <p className="text-xs text-[#6f6b77] mt-0.5">${emp.payUsd.toLocaleString()}/mo</p>
                        </div>
                      </div>
                    ))}
                    {valid.length > 5 && (
                      <div className="px-4 py-2.5 text-center text-xs text-[#6f6b77] bg-[#1a1821]">
              ...
              {errors.length > 0 && (
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-widest text-[#6f6b77] mb-2">Errors (skipped)</p>
              ...
              {valid.length === 0 && (
                <p className="text-sm text-[#6f6b77] text-center py-2">No valid employees found. Fix the errors and try again.</p>
              )}
            </>
          )}
          ...
          {step === ImportStep.Done && (
            <div className="flex flex-col items-center justify-center py-8 gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-[#6f6b77] mt-1">
        ...
        {/* Footer */}
        <div className="flex justify-end gap-3 px-6 py-4 bg-[#1a1821] border-t border-[#2e2d38]">
          {step === ImportStep.Upload && (
            <button
              onClick={handleClose}
              className="px-5 py-2.5 rounded-lg text-sm font-medium text-[#6f6b77] hover:text-white bg-white/[0.04] hover:bg-white/[0.07] border border-[#2e2d38] transition-all"
            >
              Cancel
            </button>
          )}
          {step === ImportStep.Preview && (
            <>
              <button
                onClick={handleReset}
                className="px-5 py-2.5 rounded-lg text-sm font-medium text-[#6f6b77] hover:text-white bg-white/[0.04] hover:bg-white/[0.07] border border-[#2e2d38] transition-all flex items-center gap-1.5"
              >
