  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-2xl overflow-hidden shadow-2xl bg-white dark:bg-[#15141a] border border-gray-200 dark:border-[#2e2d38]">
        ...
        <div className="p-6 space-y-4 max-h-[65vh] overflow-y-auto">
          ...
          {error && !isSuccess && (
            <div className="p-4 rounded-xl bg-red-500/[0.08] border border-red-500/20">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-red-600 dark:text-red-400 mb-0.5">Transaction Failed</p>
                  <p className="text-xs text-gray-500 dark:text-[#6f6b77]">{error}</p>
                </div>
              </div>
            </div>
          )}
          ...
          {isProcessing && !isSuccess && (
            <div className="p-4 rounded-xl bg-[#5D00F2]/[0.08] border border-[#5D00F2]/20">
              <div className="flex items-center gap-3">
                <Loader2 className="w-5 h-5 text-purple-500 dark:text-purple-400 animate-spin shrink-0" />
                <div>
          ...
          {!isSuccess && !isProcessing && (
            <>
                    ...
                    <button
                      ...
                      className={[
                        ...
                        selectedCurrency === cur
                          ...
                          : 'text-gray-500 dark:text-[#6f6b77] hover:text-gray-900 dark:hover:text-white',
                      ].join(' ')}
                    >
                      {cur}
                      <span className={`block text-[10px] font-normal mt-0.5 ${selectedCurrency === cur ? 'text-white/60' : 'text-gray-400 dark:text-[#44414c]'}`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 rounded-xl bg-gray-50 dark:bg-[#1a1821] border border-gray-200 dark:border-[#2e2d38]">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="text-[11px] font-semibold uppercase tracking-widest text-gray-500 dark:text-[#6f6b77]">Total</span>
                  </div>
                ...
                <div className="p-4 rounded-xl bg-gray-50 dark:bg-[#1a1821] border border-gray-200 dark:border-[#2e2d38]">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="text-[11px] font-semibold uppercase tracking-widest text-gray-500 dark:text-[#6f6b77]">Gas Saved</span>
                  </div>
                  <p className="text-lg font-bold text-cyan leading-tight">{gasSavings}</p>
                  <p className="text-xs text-gray-500 dark:text-[#6f6b77] mt-1">vs individual txns</p>
                </div>
              </div>
              ...
              {employees.length <= 5 && (
                <div>
                  <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-500 dark:text-[#6f6b77] mb-2">Recipients</p>
                  <div className="rounded-xl border border-gray-200 dark:border-[#2e2d38] overflow-hidden divide-y divide-gray-100 dark:divide-[#2e2d38]/60">
                    {employees.map(emp => (
                      <div key={emp.id} className="flex items-center justify-between px-4 py-3 bg-gray-50 dark:bg-[#1a1821]">
                        <div>
                          <p className="text-sm font-medium text-gray-900 dark:text-white">{emp.name}</p>
                          <p className="text-xs text-gray-500 dark:text-[#6f6b77]">{emp.role}</p>
                        </div>
              ...
              {employees.length > 5 && (
                <div className="flex items-center justify-between px-4 py-3 rounded-xl bg-gray-50 dark:bg-[#1a1821] border border-gray-200 dark:border-[#2e2d38]">
                  <p className="text-sm text-gray-700 dark:text-[#c4bfce]">{employees.length} employees selected</p>
        ...
        {/* Footer */}
        {!isSuccess && (
          <div className="flex gap-3 px-6 py-4 bg-gray-50 dark:bg-[#1a1821] border-t border-gray-200 dark:border-[#2e2d38]">
            <button
              onClick={handleClose}
              disabled={isProcessing}
              className="px-5 py-2.5 rounded-lg text-sm font-medium text-gray-500 dark:text-[#6f6b77] hover:text-gray-900 dark:hover:text-white bg-black/[0.04] dark:bg-white/[0.04] hover:bg-black/[0.07] dark:hover:bg-white/[0.07] border border-gray-200 dark:border-[#2e2d38] transition-all disabled:opacity-50"
            >
              Cancel
            </button>
            <button
